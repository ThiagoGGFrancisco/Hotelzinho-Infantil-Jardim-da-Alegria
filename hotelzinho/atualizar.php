...
//SELECIONAR
	$selecao = "SELECT * FROM imagem WHERE	id = ".$id."";
	$linhas = mysqli_query($conexao, $selecao);
	if($linhas):
	...
	if ($error > 0){
		...
		if (array_search($type, $tiposPermitidos) === false) {
		  ...
		  die("Envie apenas imagens GIF, JPEG, PJPEG e PNG");
			...
			header("location: sistema.php?tamanhoFoto=nok");
			...
			if(!empty($_POST['nome']) && !empty($_POST['email']) && !empty($_POST['observacao'])){
				...
				//ATUALIZAR
				$queryAtualizar = "UPDATE imagem SET nome='".$nome."', email='".$email."', observacao='".$observacao."', foto='".$foto."' WHERE id='".$id."' ";
				$atualizar = mysqli_query($conexao_db, $queryAtualizar);

				if($atualizar):
					mysqli_close($conexao_db);
					header("location: sistema.php?atualizado=ok");
				else:
					mysqli_close($conexao_db);
					header("location:  sistema.php?atualizado=nok");
				endif;
			}else{
				mysqli_close($conexao_db);
				header("location:  sistema.php?campos=nok");
			}
		}	
	}
?>
