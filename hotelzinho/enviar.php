		...
		if (array_search($type, $tiposPermitidos) === false) {
		  ...
		  die("Envie apenas imagens GIF, JPEG, PJPEG e PNG");
			...
			if(!empty($_POST['nome']) && !empty($_POST['email']) && !empty($_POST['observacao'])){
				...
				//INCLUIR
				$queryInclui = "INSERT INTO imagem (nome, email, observacao, foto) VALUES ('".$nome."','".$email."', '".$observacao."', '".$foto."')";
				$inclui = mysqli_query($conexao,$queryInclui);
				if($inclui):
					mysqli_close($conexao);
					header("location: sistema.php?enviado=ok");
				else:
					mysqli_close($conexao);
					header("location: sistema.php?enviado=nok");
				endif;
			}else{
				mysqli_close($conexao);
